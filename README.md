...
Print out all the 3D parts listed in the project
Note: You might need to make some modifications to the Schleuse (gate/lock mechanism) depending on your specific servo choice
Important: Use a stronger servo for the Schleuse mechanism to ensure reliable operation
...
Stepper Motor Speed (PWM)	1	GPIO 	PWM0	Hardware PWM for speed control
Stepper Motor Direction	0	GPIO 	GPIO	0=Up, 1=Down
Lower Limit Sensor	2	GPIO 	Input	Normally Open (1=triggered)
Upper Limit Sensor	27	GPIO Input	Pull-Down configuration
Sorting Servo	23	GPIO 	PWM1	Hardware PWM, 270° servo
Gate/Schleuse Servo	6	GPIO 	GPIO	Software PWM for gate mechanism
Vibration Motor 1	4	GPIO 	GPIO	Software PWM
Vibration Motor 2	5	GPIO 	GPIO	Software PWM
Vibration Kill Pin	8	GPIO Output	Emergency stop for lower vibration motor
LED Strip (SPI Alternative)	12	GPIO 	MOSI (SPI0)
⚠️ Important Notes:
...
Make sure the lego_colors.CSV or lego_colors.xlsx file is in the same directory as the script
...
📋 Assembly Notes
Schleuse Mechanism: The gate/lock mechanism (Schleuse) may require adjustments depending on your servo choice. Consider using a stronger servo for reliable operation.

Servo Selection: Standard hobby servos may not provide enough torque. Use stronger servos for critical movement components, especially the Schleuse.
...
🔗 External Services
This project uses the Brickognize API for LEGO brick recognition.
